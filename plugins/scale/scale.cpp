...
public:
  void init() override {
    ...
    grab =
        std::make_unique<wf::input_grab_t>("scale", output, this, this, this);
  ...
  void check_focus_view(wayfire_toplevel_view view) {
    if (view == current_focus_view) {
      current_focus_view =
          toplevel_cast(wf::get_active_view_for_output(output));
    }
  ...
  bool should_scale_view(wayfire_toplevel_view view) {
    ...
    return std::find(views.begin(), views.end(), get_top_parent(view)) !=
           views.end();
  }
  ...
  void handle_view_disappeared(wayfire_toplevel_view view) {
    if (scale_data.count(get_top_parent(view)) != 0) {
      /* Note: this signal can be emitted both when a view is removed from an
       * output (closed or moved to another output) and when it is minimized. We
       * use should_scale_view() to distinguish between the two cases. This only
       * matters if we show minimized views. */
